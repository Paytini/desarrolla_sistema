...
const server = http.createServer(async (req, res) => {
  req.on("error", () => {})
  res.on("error", () => {})
  if (req.headers["x-d360-portal-key"] !== config.bridgeKey)
    return send(res, 401, { message: "bad key" })
  if (Math.random() < config.bridgeErrorRate)
    return send(res, 500, { message: "lt-mock injected error" })
  await delay()

  const url = new URL(req.url, `http://localhost:${config.bridgePort}`)
  const path = url.pathname.replace(PREFIX, "")
  const studentMatch = path.match(
    /^\/students\/(\d+)\/(courses|certificates|access\/ensure|diagnostics)$/,
  )

  if (path === "/health") {
    return send(res, 200, {
      ok: true,
      plugin_version: "lt-mock-1.0",
      site_url: `http://localhost:${config.bridgePort}`,
      wordpress_version: "6.5",
      tutor_rest_available: true,
      service_user_configured: true,
      learning_webhook_configured: true,
    })
  }
  ...
  if (path === "/enrollments/batch" && req.method === "POST") {
    ...
    return send(res, 200, {
      ...
      enrolled_course_ids: body.course_ids,
      failed_course_ids: [],
    })
  }
  if (studentMatch) {
    ...
    if (kind === "access/ensure" && req.method === "POST") {
      ...
      return send(res, 200, {
        ...
        completed_course_ids: body.course_ids,
        already_active_ids: [],
        failed_course_ids: [],
      })
    }
    if (kind === "courses") {
      ...
      const courses = ids.map((id) => ({
        wp_course_id: id,
        title: courseNames[id] ?? `Curso ${id}`,
        progress_pct: (studentId + id) % 101,
        completed: (studentId + id) % 101 === 100,
        started_at: null,
        completed_at: null,
    ...
    if (kind === "diagnostics")
      return send(res, 200, {
        ...
        plugin_version: "lt-mock-1.0",
        service_user_id: 1,
        tutor_courses_raw: null,
        tutor_courses_count: 0,
        direct_courses_count: 0,
        courses: [],
      })
  }
  if (path === "/courses") {
    const courses = Object.entries(courseNames).map(([id, title]) => ({
      wp_course_id: Number(id),
      title,
      status: "publish",
      post_type: "courses",
      course_url: null,
...
server.listen(config.bridgePort, () =>
  console.log(
    `lt-mock-bridge en http://localhost:${config.bridgePort}${PREFIX} (latencia ${config.bridgeLatencyMs}ms, frio ${config.bridgeColdRate * 100}%)`,
  ),
)
