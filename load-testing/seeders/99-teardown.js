...
const STEPS = [
  ...
  ["employee_courses", `DELETE FROM employee_courses WHERE employee_id IN (${seededEmployees})`, true],
  ...
  ["consulting_requests", `DELETE FROM consulting_requests WHERE requested_by_user_id IN (${seededUsers}) OR company_id IN (${seededCompanies})`, true],
  ...
  ["company_packages", `DELETE FROM company_packages WHERE company_id IN (${seededCompanies})`, false],
  ["companies", `DELETE FROM companies WHERE slug LIKE 'lt-%'`, false],
  ["package_courses", `DELETE FROM package_courses WHERE package_id IN (SELECT id FROM packages WHERE name LIKE 'LT %')`, false],
  ["company_packages(pkg)", `DELETE FROM company_packages WHERE package_id IN (SELECT id FROM packages WHERE name LIKE 'LT %')`, false],
  ["course_dc3_metadata", `DELETE FROM course_dc3_metadata WHERE wp_course_id BETWEEN 900101 AND 900199`, false],
