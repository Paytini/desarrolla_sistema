...
async function main() {
  ...
  for (let i = 0; i < config.scale.companies; i++) {
    ...
    const cp = await query(`SELECT id FROM company_packages WHERE company_id = $1 AND package_id = $2 AND active`, [companyId, catalog.packageId])
    if (cp.rows.length === 0) {
      await query(`INSERT INTO company_packages (company_id, package_id, start_date, active, created_at) VALUES ($1, $2, now(), true, now())`, [companyId, catalog.packageId])
    }
  ...
  const csv = ["email,password,slug", ...out.map((c) => `${c.rhEmail},${config.seededPassword},${c.slug}`)].join("\n")
