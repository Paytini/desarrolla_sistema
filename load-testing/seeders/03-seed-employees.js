...
async function main() {
  ...
  for (const company of companies) {
    ...
    for (let i = 0; i < config.scale.employeesPerCompany; i++) {
      ...
      employeeRows.push([company.id, wpUserId, names.firstName(globalIdx), names.lastName(globalIdx), email, names.curp(globalIdx), "Operaciones", "Analista", "07.2", "Supervision de seguridad", true])
    ...
    for (let start = 0; start < employeeRows.length; start += CHUNK) {
      ...
      await bulkInsert(
        "employees",
        ["company_id", "wp_user_id", "first_name", "last_name", "email", "curp", "department", "position", "occupation_code", "occupation_name", "active", "created_at", "updated_at"],
        chunk.map((r) => [...r, now, now]),
        "ON CONFLICT (email) DO NOTHING"
      )
      await bulkInsert(
        "users",
        ["email", "password_hash", "role", "name", "company_id", "wp_user_id", "active", "created_at", "updated_at"],
        chunk.map((r) => [r[4], hash, "EMPLEADO", `${r[2]} ${r[3]}`, company.id, r[1], true, now, now]),
        "ON CONFLICT (email) DO NOTHING"
      )
    }
    ...
    ids.rows.forEach((emp, idx) => {
      catalog.courseIds.forEach((courseId, c) => {
        ...
        courseRows.push([emp.id, courseId, `LT Curso Masivo ${c + 1}`, completed ? 100 : progress, completed, "ACTIVE", "DIRECT_ENROLLMENT", completed ? new Date() : null])
      })
    ...
    for (let start = 0; start < courseRows.length; start += CHUNK) {
      await bulkInsert("employee_courses", ["employee_id", "wp_course_id", "course_name", "progress_pct", "completed", "access_status", "access_source", "completed_at"], courseRows.slice(start, start + CHUNK), "ON CONFLICT (employee_id, wp_course_id) DO NOTHING")
    }
    await bulkInsert("certificates", ["employee_id", "wp_course_id", "course_name", "reference_number"], certRows, "ON CONFLICT (reference_number) DO NOTHING")
    await query(`UPDATE companies SET used_seats = (SELECT count(*) FROM employees WHERE company_id = $1 AND active) WHERE id = $1`, [company.id])
    console.log(`empresa ${company.slug}: ${ids.rows.length} empleados`)
  }

  // resolver ids reales de certificados para el payload
  const certs = await query(`SELECT c.id, c.reference_number, e.email FROM certificates c JOIN employees e ON e.id = c.employee_id WHERE c.reference_number LIKE 'LT-D360-%'`)
  const byRef = new Map(certs.rows.map((r) => [r.reference_number, r]))
  const resolved = certLines.map((line) => {
    const m = line.match(/^__CERT_(.+)__,(.+),(.+)$/)
    if (!m) return line
    const row = byRef.get(m[1])
    return row ? `${row.id},${m[2]},${m[3]}` : null
  }).filter(Boolean)
  fs.writeFileSync(path.join(config.dataDir, "payloads", "employee-credentials.csv"), credLines.join("\n"))
