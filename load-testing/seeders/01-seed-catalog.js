...
async function main() {
  ...
  for (let i = 0; i < config.namespace.courseCount; i++) {
    ...
    dc3Rows.push([courseId, `LT Curso Masivo ${i + 1}`, 8, "Seguridad", "07.2", "Desarrolla360 LT", "LT-REG-001", "Instructor LT", now, now])
  }
  await bulkInsert("package_courses", ["package_id","wp_course_id","course_name","lesson_count"], courseRows, "ON CONFLICT (package_id, wp_course_id) DO NOTHING")
  await bulkInsert(
    "course_dc3_metadata",
    ["wp_course_id","course_name","duration_hours","subject_area_name","subject_area_code","training_agent_name","training_agent_registration","instructor_name","created_at","updated_at"],
    dc3Rows.map((r) => r),
    "ON CONFLICT (wp_course_id) DO NOTHING"
  )
  fs.writeFileSync(path.join(config.dataDir, "catalog.json"), JSON.stringify({ packageId, courseIds }))
