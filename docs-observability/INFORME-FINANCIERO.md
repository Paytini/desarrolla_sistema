...
> ⚠️ Los costos de uso (cómputo, requests, correos) son **estimaciones** derivadas del modelo de tráfico de cada fase. Los precios unitarios sí son los oficiales vigentes. Configurar _Spend Management_ en Vercel (alertas + pausa automática) desde el día 1.
...
_Lectura:_ hoy la infraestructura es barata — el problema de F0 no es el costo sino que **se cae** con los límites descritos en el informe de rendimiento (pooler de 200 clientes con Micro, timeouts, etc.).
...
_Palanca de ahorro:_ el mayor rubro variable son los **edge requests** (assets estáticos). Mitigación gratuita: `Cache-Control: immutable` largo en assets (Next ya lo hace) y evitar re-navegaciones innecesarias (el `router.refresh()` del polling actual re-descarga payloads RSC — arreglarlo en G-4 también ahorra dinero).
...
_Lectura:_ aquí la DB pasa a ser el rubro #1. La alternativa a subir de tier sería Prisma Accelerate, pero a 15–30 M queries/mes costaría $130–500 — **subir el tier de Supabase es más barato y más simple.**
...
_Nota WordPress:_ es el único costo que no controla el portal. El streaming de video ya lo absorbe WP (decisión correcta — ver informe de rendimiento §2); con CDN de Cloudflare delante, la mayor parte de ese tráfico pasa a la caché y el hosting WP no necesita crecer proporcionalmente a los usuarios.
