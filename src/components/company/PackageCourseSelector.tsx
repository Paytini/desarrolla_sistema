...
export default function PackageCourseSelector({
  ...
  useEffect(() => {
    ...
    async function loadCourses() {
      ...
      try {
        ...
        if (active) {
          const fetched = (payload.courses ?? []).filter((c) =>
            c.status ? c.status === "publish" : true,
          )
  ...
  const filteredCourses = useMemo(() => {
    ...
    if (!q) return courses
    return courses.filter((c) => `${c.wp_course_id} ${c.title}`.toLowerCase().includes(q))
  }, [courses, query])
    ...
    <Box sx={{ display: "grid", gap: 1.5 }}>
      ...
      >
        ...
        <Typography variant="body2" sx={{ color: "text.secondary", flexShrink: 0 }}>
          {selectedIds.length} curso{selectedIds.length === 1 ? "" : "s"} seleccionado
              ...
              return (
                <Box
                  ...
                  sx={{
                    ...
                    borderRadius: "10px",
                    border: "1px solid",
                    borderColor: checked ? "rgba(53,121,245,0.3)" : "divider",
                    bgcolor: checked ? "rgba(53,121,245,0.04)" : "background.paper",
                    cursor: "pointer",
