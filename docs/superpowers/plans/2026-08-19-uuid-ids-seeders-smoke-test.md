...
Spec §8 also calls for: "login as all 3 roles, view a company, view/download a certificate ..., upload a company logo and an instructor signature ..., trigger a package sync ..., edit DC-3 metadata ...". This cannot be scripted from an agent session: `auth.ts` calls `verifyTurnstileToken` unconditionally against Cloudflare's real `siteverify` endpoint whenever `TURNSTILE_SECRET_KEY` is set (confirmed by reading `lib/turnstile.ts` — no dev/test bypass exists), and the `.env` in this worktree has real, non-test Cloudflare Turnstile keys configured. There is no way to pass the login captcha without a real browser and a human solving it (or Cloudflare's invisible-mode heuristics passing it, which cannot be relied on from a headless/cloud agent).
...
3. **SUPERADMIN or RH → download the seeded certificate** (`D360-2026-0416-001`, reference-number search or via the employee's certificate list). Confirms the Vercel Blob path (`` `constancias/${companyId}/${employeeId}/dc3-${certificateId}.pdf` ``) still resolves with UUID path segments, and the certificate's `folio_sequence` (verified in Task 1 Step 9) surfaces correctly wherever the UI displays the folio.
4. **SUPERADMIN → upload a company logo** (`/superadmin/companies/{id}`) and **upload an instructor signature** (wherever `curso_dc3_metadata` is edited, `/superadmin/dc3`). Confirms both `app/api/upload/company-logo/route.ts` (the route Plan 3 fixed for FormData UUID parsing) and `app/api/upload/instructor-signature/route.ts` accept the UUID company/course IDs correctly.
5. **SUPERADMIN → assign the demo package to a second company, or trigger a re-sync on the existing assignment** (`/superadmin/packages`, or `/superadmin/reports`'s retry-sync action). Confirms `lib/course-sync.ts`/`lib/jobs.ts`'s `Job.payload` (`companyId: string`, `employeeIds: string[]`) round-trips correctly through a real chunked async job run.
6. **SUPERADMIN → edit DC-3 metadata for a course** (`/superadmin/dc3`) and confirm the change is reflected immediately (cache invalidation via `lib/cache-tags.ts` still matches on the new UUID-keyed tags).

Report back per-item pass/fail; anything that fails here is a real bug in Plans 1-3's work, not a Plan 4 defect, and should be triaged before considering the whole UUID migration done.
